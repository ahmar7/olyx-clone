import React from "react";
import "./perks.css";
const Perks = () => {
  return (
    <div>
      <section className="section home-perks wf-section">
        <div className="container-default w-container">
          <div className="home-perks-wrapper">
            <div className="home-perks-content-top-wrapper">
              <div className="home-perks-content-top-content">
                <br />
                <h2 className="title home-perks text-color-accent">
                  Look ahead!
                </h2>
                <p className="cavt paragraph  home-perks">
                  Inflation is rampant in today’s economy and will quickly eat
                  away at your savings. In times when asset prices fall rapidly
                  and show no signs of stopping, we must look for a stable
                  alternative to preserve our wealth and create cash flow.
                </p>
                <p className="cavt paragraph  home-perks">
                  Many people have yet to realize, with the right strategies and
                  automation, the true power of DeFi & AI as the ultimate way to
                  generate stable and passive yield regardless of the economic
                  environment. Make sure you’re a part of this revolution when
                  it is still broadly unrecognised!
                </p>

                <p className="cavt paragraph  home-perks">
                  The graph on the right showcases how a modest investment with
                  YieldTrust can quickly grow with compounding to a significant
                  amount compared to leaving the funds for this investment in a
                  bank.
                </p>
              </div>
              <div
                data-w-id="24a66438-d00d-342a-e10f-1469d5035cdc"
                data-aos="fade-up"
                data-aos-offset="200"
                data-aos-delay="50"
                data-aos-duration="1200"
                data-aos-easing="ease-in"
                data-aos-mirror="false"
                data-aos-once="true"
                className="image-wrapper home-perks-content-top-image"
              >
                <div
                  data-w-id="896af325-1bd1-61b2-6922-019b2a2de228"
                  className="div-block technologies charts"
                >
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de229-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de229"
                    className="chart _1"
                    style={{ height: 19 }}
                  />
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de22a-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de22a"
                    className="chart _2"
                    style={{ height: 19 }}
                  />
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de22b-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de22b"
                    className="chart _3"
                    style={{ height: 19 }}
                  />
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de22c-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de22c"
                    className="chart _4"
                    style={{ height: 19 }}
                  />
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de22d-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de22d"
                    className="chart _5"
                    style={{ height: 19 }}
                  />
                  <div
                    id="w-node-_896af325-1bd1-61b2-6922-019b2a2de22e-11098043"
                    data-w-id="896af325-1bd1-61b2-6922-019b2a2de22e"
                    className="chart _6"
                    style={{ height: 19 }}
                  />
                </div>
              </div>
            </div>
            <div className="home-perks-content-bottom-wrapper">
              <div
                data-w-id="d33741e8-edd1-da97-3b24-7982c1abaa36"
                style={{
                  transform:
                    "translate3d(0px, 40px, 0px) scale3d(1, 1, 1)  rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)",
                  transformStyle: "preserve-3d",
                  opacity: 1,
                }}
                className="image-wrapper home-perks-content-bottom-image"
              />
              <div className="home-perks-content-bottom-content" />
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Perks;
